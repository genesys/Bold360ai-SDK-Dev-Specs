...
static NSString *const BC_SRWebSocketAppendToSecKeyString = @"258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

static inline int32_t validate_dispatch_data_partial_string(NSData *data);
static inline void SRFastLog(NSString *format, ...);

@interface NSData (BC_SRWebSocket)
...
@interface NSString (BC_SRWebSocket)
...
@interface NSURL (BC_SRWebSocket)
...
@interface _BC_SRRunLoopThread : NSThread
...
@implementation NSData (BC_SRWebSocket)
...
@implementation NSString (BC_SRWebSocket)
...
NSString *const BC_SRWebSocketErrorDomain = @"BC_SRWebSocketErrorDomain";

// Returns number of bytes consumed. Returning 0 means you didn't match.
// Sends bytes to callback handler;
typedef size_t (^stream_scanner)(NSData *collected_data);

typedef void (^data_callback)(BC_SRWebSocket *webSocket,  NSData *data);

@interface BC_SRIOConsumer : NSObject {
...
@interface BC_SRIOConsumerPool : NSObject

- (id)initWithBufferCapacity:(NSUInteger)poolSize;

- (BC_SRIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
- (void)returnConsumer:(BC_SRIOConsumer *)consumer;

@end

@interface BC_SRWebSocket ()  <NSStreamDelegate> {
...
@implementation BC_SRWebSocket {
    ...
    __strong BC_SRWebSocket *_selfRetain;
    
    NSArray *_requestedProtocols;
    BC_SRIOConsumerPool *_consumerPool;
}
...
- (id <BC_SRWebSocketDelegate>)delegate {
    @synchronized(_delegateSyncObject) {
        return _delegate;
    }
}

- (void)setDelegate:(id<BC_SRWebSocketDelegate>)delegate {
    @synchronized(_delegateSyncObject) {
...
- (id)initWithURLRequest:(NSURLRequest *)request protocols:(NSArray *)protocols;
{
    ...
    if (self) {
        assert(request.URL);
        _delegateSyncObject = [[NSObject alloc] init];
        _url = request.URL;
        _urlRequest = request;
...
- (void)_SR_commonInit;
{
    ...
    _consumerPool = [[BC_SRIOConsumerPool alloc] init];
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
    ...
    NSString *concattedString = [_secKey stringByAppendingString:BC_SRWebSocketAppendToSecKeyString];
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        [self _failWithError:[NSError errorWithDomain:BC_SRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Invalid Sec-WebSocket-Accept response"] forKey:NSLocalizedDescriptionKey]]];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            [self _failWithError:[NSError errorWithDomain:BC_SRWebSocketErrorDomain code:2133 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Server specified Sec-WebSocket-Protocol that wasn't requested"] forKey:NSLocalizedDescriptionKey]]];
            return;
        }
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(BC_SRWebSocket *self,  NSData *data) {
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(BC_SRWebSocket *self, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(BC_SRWebSocket *self, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [self _addConsumerWithDataLength:extra_bytes_needed callback:^(BC_SRWebSocket *self, NSData *data) {
...
- (BOOL)_innerPumpScanner {
    ...
    BC_SRIOConsumer *consumer = [_consumers objectAtIndex:0];
...
@implementation BC_SRIOConsumer
...
@implementation BC_SRIOConsumerPool {
...
- (BC_SRIOConsumer *)consumerWithScanner:(stream_scanner)scanner handler:(data_callback)handler bytesNeeded:(size_t)bytesNeeded readToCurrentFrame:(BOOL)readToCurrentFrame unmaskBytes:(BOOL)unmaskBytes;
{
    BC_SRIOConsumer *consumer = nil;
    if (_bufferedConsumers.count) {
        consumer = [_bufferedConsumers lastObject];
        [_bufferedConsumers removeLastObject];
    } else {
        consumer = [[BC_SRIOConsumer alloc] init];
    }
...
@implementation NSURL (BC_SRWebSocket)
...
static _BC_SRRunLoopThread *networkThread = nil;
static NSRunLoop *networkRunLoop = nil;

@implementation NSRunLoop (BC_SRWebSocket)

+ (NSRunLoop *)SR_networkRunLoop {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        networkThread = [[_BC_SRRunLoopThread alloc] init];
...
@implementation _BC_SRRunLoopThread {
